...
// through the given day. (Do take leap years into account.)
...
int days_heap(int d, char * m, int y)
{
    ...
    for (int i = 0; i < 12; i++) {
        if (strcmp(m, months_heap[i].name) == 0 ||
            strcmp(m, months_heap[i].abbr) == 0 ||
            atoi(m) == months_heap[i].monumb)
        {
