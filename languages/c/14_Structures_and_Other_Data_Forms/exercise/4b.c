...
// to the following criteria:
    ...
    // structure with three members. Its first member contains a first name,
    // its second member contains a middle name, and its final member contains
    // a last name. Create and initialize an array of five such structures.
    // Have the program print the data in this
...
void show_in_struct_member(char *, char *, char *, int);
...
int main(void)
{
    ...
    for (int i = 0; i < 5; i++)
        show_in_struct_member(peoples[i].fname, peoples[i].middle,
                           peoples[i].lname, peoples[i].security_num);

    return 0;
}

void show_in_struct_member(char * fname, char * middle,
                           char * lname, int security_num)
{
    printf("%s, %s %c%c -- %d\n", fname, lname, *middle,
                                  *middle ? '.' : '\b', security_num);
}
